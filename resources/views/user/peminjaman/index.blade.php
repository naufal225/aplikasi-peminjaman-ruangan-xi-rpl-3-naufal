<x-layout-user>
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        <div class="border-b border-gray-200 mb-6">
            <nav class="-mb-px flex space-x-6">
                <a href="{{ route('user.peminjaman.index', ['status' => 'semua']) }}"
                    class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm {{ $status === 'semua' || !$status ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                    Semua Peminjaman
                </a>
                <a href="{{ route('user.peminjaman.index', ['status' => 'menunggu']) }}"
                    class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm {{ $status === 'menunggu' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                    Menunggu Persetujuan
                </a>
                <a href="{{ route('user.peminjaman.index', ['status' => 'disetujui']) }}"
                    class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm {{ $status === 'disetujui' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                    Disetujui
                </a>
                <a href="{{ route('user.peminjaman.index', ['status' => 'ditolak']) }}"
                    class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm {{ $status === 'ditolak' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                    Ditolak
                </a>
                <a href="{{ route('user.peminjaman.index', ['status' => 'selesai']) }}"
                    class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm {{ $status === 'selesai' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                    Selesai
                </a>
            </nav>
        </div>
        ...
        <div class="mb-6">
            <div class="relative">
                <input type="text" id="searchInput" placeholder="Cari berdasarkan nama ruangan..."
                    value="{{ $search ?? '' }}"
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $item->ruangan->nama_ruangan ?? 'Ruang
                                Rapat Utama' }}</div>
                            <div class="text-xs text-gray-500">{{ $item->ruangan->lokasi ?? 'Lantai 2, Gedung A' }}
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <div>{{ $item->waktu_mulai_formatted ?? '09:00' }} - {{ $item->waktu_selesai_formatted ??
                                '12:00' }}</div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                <a href="{{ route('user.peminjaman.show', $item->id ?? 1) }}"
                                ...
                                <a href="{{ route('user.pengembalian.create', $item->id ?? 1) }}"
                                    class="text-green-600 hover:text-green-900" title="Ajukan Pengembalian">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <button onclick="confirmCancel({{ $item->id ?? 1 }})"
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <div>09:00 - 12:00</div>
                            <div class="text-xs text-gray-500">3 jam</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                <a href="{{ route('user.peminjaman.show', 1) }}"
                                    class="text-blue-600 hover:text-blue-900" title="Detail">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <a href="{{ route('user.pengembalian.create', 1) }}"
                                    class="text-green-600 hover:text-green-900" title="Ajukan Pengembalian">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <div>13:00 - 15:00</div>
                            <div class="text-xs text-gray-500">2 jam</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                <a href="{{ route('user.peminjaman.show', 2) }}"
                                    class="text-blue-600 hover:text-blue-900" title="Detail">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <button
                                    onclick="confirmCancel('{{ route('user.peminjaman.cancel', $peminjaman->id) }}')"
                                    class="text-white-600 hover:text-yellow-900" title="Batalkan">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M6 18L18 6M6 6l12 12" />
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                .then(data => {
                    Swal.fire({
                        title: 'Dibatalkan!',
                        text: data.message ?? 'Peminjaman berhasil dibatalkan.',
                        icon: 'success',
                        timer: 2000,
